...
START = "<!-- BIOLOGY_EXAM_1445_CHECKPOINT_START -->"
END = "<!-- BIOLOGY_EXAM_1445_CHECKPOINT_END -->"
...
def main():
    ...
    if recon.get("individual_exam_model_count") != 20 or recon.get("finalized_exam_page_count") != 60:
        raise SystemExit("Biology 1445 model/page count not finalized")
    qmap = recon.get("question_mapping") or {}
    if any(int(qmap.get(k, -1)) != 0 for k in ("exam_linked_structural", "review_required", "unassigned_within_source")):
        raise SystemExit("Biology 1445 must have zero legacy question mappings")

    master = json.loads(MASTER_PATH.read_text(encoding="utf-8"))
    sources = master.get("sources") or []
    current_index = next((i for i, s in enumerate(sources) if s.get("id") == SOURCE_ID), None)
    if current_index is None:
        raise SystemExit("Biology 1445 source missing from master")
    ...
    source.update({
        ...
        "classification_evidence": "legacy exam label + 60/60 technical verification + exact repository-reference identity + complete 60-page visual review resolving twenty source-local 3-page question/question/correction occurrences",
    ...
    progress["individual_exam_models"] = int(progress.get("individual_exam_models") or 0) + (20 - old_models)
    progress["exam_pages_completed"] = int(progress.get("exam_pages_completed") or 0) + (60 - old_pages)
    progress["correction_sheet_candidates"] = int(progress.get("correction_sheet_candidates") or 0) + (20 - old_corrections)
    progress["source_images_technically_verified"] = int(progress.get("source_images_technically_verified") or 0) + (60 - old_images)
    for key, value in {
    ...
    p = progress
    body = f"""## Reconstruction checkpoint — Biology Ministry Exams 1445

- Phase: `CONTENT RECONSTRUCTION + EXAM BOUNDARY DISCOVERY`
- Source `{SOURCE_ID}` — `الاحياء نماذج وزارية 1445` completed from source-local evidence.
