...
START = "<!-- ENGLISH_EXAM_1446_CHECKPOINT_START -->"
END = "<!-- ENGLISH_EXAM_1446_CHECKPOINT_END -->"
...
def main():
    ...
    if recon.get("individual_exam_model_count") != 13 or recon.get("finalized_exam_page_count") != 39:
        raise SystemExit("English 1446 model/page count not finalized")
    ...
    if int(qmap.get("exam_linked_structural", -1)) != 0 or int(qmap.get("review_required", -1)) != 0 or int(qmap.get("unassigned_within_source", -1)) != 0:
        raise SystemExit("English 1446 zero-question mapping mismatch")

    master = json.loads(MASTER_PATH.read_text(encoding="utf-8"))
    sources = master.get("sources") or []
    current_index = next((i for i, s in enumerate(sources) if s.get("id") == SOURCE_ID), None)
    if current_index is None:
        raise SystemExit("English 1446 source missing from master")
    ...
    p = progress
    body = f"""## Reconstruction checkpoint — Grade 9 English Ministry Exams 1446

- Phase: `CONTENT RECONSTRUCTION + EXAM BOUNDARY DISCOVERY`
- Source `{SOURCE_ID}` — `الانجليزي نماذج وزارية 1446` completed from source-local evidence.
