...
MASTER = ROOT / 'master-reference/تاسع إجتماعيات/التربية_الوطنية_تاسع'
...
if manifest['counts']['pages'] != 59 or manifest['counts']['image_references'] != 59:
    raise SystemExit(f"unexpected civics source count: {manifest['counts']}")
if manifest['counts']['questions'] != 0:
    raise SystemExit('civics source unexpectedly contains legacy questions')
if any(manifest['anomalies'].get(k) for k in manifest['anomalies']):
    raise SystemExit(f"civics manifest anomalies must be reviewed first: {manifest['anomalies']}")
...
if len(page_map) != 59:
    raise SystemExit('civics exact identity count is not 59')
...
if stored != list(range(min(stored), max(stored) + 1)):
    raise SystemExit('stored civics pages are not contiguous')
if source != list(range(min(source), max(source) + 1)):
    raise SystemExit('master source pages are not contiguous')
if any(r['legacy_question_count'] for r in page_map):
    raise SystemExit('pages.json contains unexpected civics AI questions')

# Build exact section/title runs from byte-identical master pages.
sections = []
for row in page_map:
    ...
    titles = sec['titles']
    if not titles or titles[-1]['title'] != row['title']:
        titles.append({
            'title': row['title'],
            'start_stored_page': row['stored_page_number'],
            'end_stored_page': row['stored_page_number'],
        ...
        titles[-1]['end_stored_page'] = row['stored_page_number']
        titles[-1]['end_source_page'] = row['source_page']

# Retained slice begins inside the exact master book. Intro pages are preserved as non-lesson pages;
# unit sections are classified only when the master section explicitly starts with "الوحدة".
units = []
lessons = []
lesson_pages = []
unit_cover_pages = []
...
for sec in sections:
    ...
    for run in title_runs[1:]:
        ...
        if title.startswith('تقويم الوحدة'):
            ...
            continue
        lesson = {
            'id': f'civics9-u{len(units)+1:02d}-l{len(unit_lessons)+1:02d}',
...
if assigned != set(stored):
    raise SystemExit(f'civics page classification mismatch: missing={sorted(set(stored)-assigned)} extra={sorted(assigned-set(stored))}')
if len(assigned) != len(lesson_pages) + len(unit_cover_pages) + len(unit_review_pages) + len(non_lesson_pages):
    raise SystemExit('civics page classifications overlap')

master_only = sorted(set(r['source_page'] for r in master_manifest) - set(source))
discovery = {
    'schema_version': 1,
    'operation': 'civics_exact_master_identity_and_structure_discovery',
    'subject_id': SID,
    'source_name': 'التربية الوطنية',
    'classification': 'educational_book_source',
    'raw_image_count': 59,
    'raw_sha_verified_count': 59,
    'master_reference_path': 'master/تاسع إجتماعيات/التربية_الوطنية_تاسع',
    'master_manifest_entries': len(master_manifest),
    'exact_sha_identity_count': 59,
...
report = {
    ...
    'source_identity': {
        ...
        'raw_master_exact_sha_matches': 59,
        'raw_images': 59,
    ...
    'evidence': [
        '59/59 retained RAW images are byte-identical by SHA-256 to unique images in the exact master civics reference.',
        'The exact master page-by-page manifest supplies section and title runs used for unit/lesson/review boundaries.',
        'The retained source contains 0 legacy AI questions; no question records were fabricated.',
    ],
    'raw_mutations': 0,
}
...
print(json.dumps({
    ...
    'covers': len(unit_cover_pages),
    'reviews': len(unit_review_pages),
    'non_lesson': len(non_lesson_pages),
    'questions': 0,
}, ensure_ascii=False))
