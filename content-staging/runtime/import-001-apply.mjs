...
const pages = [
  { order: 0, legacyPageId: '706771c2-2145-4682-9bdd-4e7df5c69bf9', path: 'public.lessons/706771c2-2145-4682-9bdd-4e7df5c69bf9/image/0', sha256: 'fc9e15f23d3f5c9a7928aecc70888e30be2c454ab16883e1a3c82290fd581fdb' },
  { order: 1, legacyPageId: '71ba9a99-e009-401f-b65f-8a956218a633', path: 'public.lessons/71ba9a99-e009-401f-b65f-8a956218a633/image/0', sha256: 'fd99f85698dd4870bf8fccf85aa53f47bf484d19b2052473155540627f6aa420' },
  { order: 2, legacyPageId: '710cb3d4-e2c1-4d05-a1ef-2c0a1bbede2b', path: 'public.lessons/710cb3d4-e2c1-4d05-a1ef-2c0a1bbede2b/image/0', sha256: '3b7415622af90557ad09585eef776b5ce3fe2c68776a1963b4a2cbf677f5ba61' },
  { order: 3, legacyPageId: '4b6090c2-744e-4fc6-8908-87c211a8713b', path: 'public.lessons/4b6090c2-744e-4fc6-8908-87c211a8713b/image/0', sha256: '3678974564e2a219840da4fc175d62d9811bd80666d01c81d4f9b49f2328e258' },
];

function assert(condition, message) {
  if (!condition) throw new Error(`IMPORT001_APPLY_FAIL: ${message}`);
}
...
await sql.begin(async (tx) => {
  ...
  for (let i = 0; i < targetAssets.length; i++) {
    ...
    assert(actual.source_path === expected.path && actual.checksum_sha256 === expected.sha256 && actual.is_present === true, `target asset provenance mismatch at ${i}`);
  ...
  const legacyAssetCounts = await tx`
    ...
    order by l.id
  `;
  assert(legacyAssetCounts.length === 4 && legacyAssetCounts.every((x) => x.asset_count === 0), 'legacy source lessons still hold reviewed page assets');
  ...
  const afterQuestions = await tx`
    ...
    order by r.id
  `;
  assert(JSON.stringify(afterQuestions) === JSON.stringify(beforeQuestions), 'legacy question rows/links changed during structural import');

  const [pub] = await tx`
    select count(*) filter (where l.published_at is not null)::int published_lessons,
...
console.log('IMPORT001_APPLY_PASS', JSON.stringify(summary));
