...
SHEETS=Path('/tmp/hadith-contact-sheets')

m=json.loads(MANIFEST.read_text())
imgs=sorted(m['images'], key=lambda x:(x.get('page_number') is None, x.get('page_number') or -1, x.get('image_index',0)))
assert len(imgs)==62, len(imgs)
assert m['counts']['questions']==1483, m['counts']['questions']
...
for r,p,h in source:
    ...
    matches.append({'page_number':r.get('page_number'),'legacy_page_id':r.get('legacy_page_id'),'sha256':h,'master_exact_sha_paths':paths})
...
for batch_start in range(0,len(source),12):
    ...
    for r,p,h in batch:
        with Image.open(p) as im:
            ...
            ImageDraw.Draw(cell).text((8,378),f"storage page {r.get('page_number')}",fill='black')
            cells.append(cell)
    ...
    sheet.save(SHEETS/f'hadith-pages-{lo:03d}-{hi:03d}.jpg',quality=88)

identity='VERIFIED_EXACT_SHA' if len(matched)==62 else 'NOT VERIFIED'
out={
 'source_id':SID,
 'source_name':'الحديث والتهذيب الكتاب',
 'status':'discovery_complete_not_finalized',
 'source_pages':62,
 'legacy_questions':1483,
 'retained_storage_page_range':[min(r['page_number'] for r in imgs),max(r['page_number'] for r in imgs)],
 'technical_source_sha_rechecked':62,
 ...
 'next_operation':'Visually inspect all 62 source pages/contact sheets and prove lesson/review boundaries from this exact source; only then link legacy questions by verified page membership.'
}
