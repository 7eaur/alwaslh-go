...
START = "<!-- SCIENCE_EXAM_1445_CHECKPOINT_START -->"
END = "<!-- SCIENCE_EXAM_1445_CHECKPOINT_END -->"
...
def main():
    ...
    if recon.get("individual_exam_model_count") != 10 or recon.get("finalized_exam_page_count") != 30:
        raise SystemExit("Science 1445 model/page count not finalized")
    qmap = recon.get("question_mapping") or {}
    linked = int(qmap.get("exam_linked_structural", -1))
    if linked != 215 or int(qmap.get("review_required", -1)) != 0 or int(qmap.get("unassigned_within_source", -1)) != 0:
        raise SystemExit("Science 1445 question mapping not complete")

    master = json.loads(MASTER_PATH.read_text(encoding="utf-8"))
    sources = master.get("sources") or []
    current_index = next((i for i, s in enumerate(sources) if s.get("id") == SOURCE_ID), None)
    if current_index is None:
        raise SystemExit("Science 1445 source missing from master")
    ...
    source.update({
        ...
        "exam_reconstruction": {
            ...
            "associated_legacy_questions": 215,
    ...
    p = progress
    body = f"""## Reconstruction checkpoint — Grade 9 Science Ministry Exams 1445

- Phase: `CONTENT RECONSTRUCTION + EXAM BOUNDARY DISCOVERY`
- Source `{SOURCE_ID}` — `العلوم نماذج وزارية 1445` completed from source-local evidence.
