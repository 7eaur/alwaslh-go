...
for batch_start in range(0,len(source),12):
    ...
    lo=batch[0][0]['page_number']; hi=batch[-1][0]['page_number']; sheet.save(SHEETS/f'physics-pages-{lo:03d}-{hi:03d}.jpg',quality=88)

identity='VERIFIED_EXACT_SHA' if len(matched)==207 and preferred_count==207 else 'NOT VERIFIED'
out={
 'source_id':SID,'source_name':'الفيزياء الكتاب المدرسي','status':'discovery_complete_not_finalized',
 'source_pages':207,'legacy_questions':3101,'retained_storage_page_range':[min(r['page_number'] for r in imgs),max(r['page_number'] for r in imgs)],
 'technical_source_sha_rechecked':207,'identity_status':identity,'exact_sha_identity_count':len(matched),'unmatched_storage_pages':unmatched,
