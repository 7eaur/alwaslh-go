...
START='<!-- PHYSICS_EXAM_1447_CHECKPOINT_START -->'; END='<!-- PHYSICS_EXAM_1447_CHECKPOINT_END -->'
...
def main():
    ...
    if r.get('individual_exam_model_count')!=31 or r.get('finalized_exam_page_count')!=124: raise SystemExit('Physics 1447 model/page count not finalized')
    q=r.get('question_mapping') or {}
    if (q.get('exam_linked_structural'),q.get('review_required'),q.get('unassigned_within_source'))!=(255,0,0): raise SystemExit('Physics 1447 question mapping not finalized')
    ...
    s.update({'classification':'exam_source_group','classification_evidence':'legacy exam label + 124/124 technical verification + complete source-local 124-page visual review resolving thirty-one four-page question/question/question/correction occurrences','review_status':'processed_boundary_verified_answer_keys_not_verified','exam_models':31,'answer_keys':'NOT VERIFIED','technical_verification':{'status':'verified','report_path':f'content-staging/reconstruction/technical/{SOURCE_ID}.json','images_verified':124,'readable':124,'sha256_match_manifest':124,'mime_match_manifest':124,'duplicate_sha_groups_within_source':9},'exam_reconstruction':{'status':r['status'],'path':f'content-staging/reconstruction/exams/source-groups/{SOURCE_ID}.json','source_blocks_reviewed':31,'verified_source_occurrences':31,'review_required_blocks':0,'individual_exam_models':31,'source_pages':124,'finalized_exam_pages':124,'review_required_pages':0,'pages_per_source_block':4,'question_pages_per_verified_occurrence':3,'correction_sheet_candidates':31,'verified_answer_keys':0,'answer_key_status':'NOT VERIFIED','exam_linked_questions':255,'review_required_questions':0,'associated_legacy_questions':255,'semantic_question_correctness':'NOT VERIFIED','raw_mutations':0}})
    p=m.setdefault('reconstruction_progress',{}); sd=0 if old_processed else 1; md=31-old_models; pd=124-old_pages; cd=31-old_corr; idelta=124-old_images; eqd=255-old_eq; rqd=0-old_rq
    ...
    body=f'''## Reconstruction checkpoint — Physics Ministry Exams 1447

- Phase: `CONTENT RECONSTRUCTION + EXAM BOUNDARY DISCOVERY`
- Source `{SOURCE_ID}` — `الفيزياء نماذج وزاريه 1447` completed from source-local evidence.
