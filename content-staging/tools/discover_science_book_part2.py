...
MASTER=ROOT/'master-reference/تاسع علوم/علوم_تاسع_الجزء_الثاني'
...
for r in rows:
 ...
 elif s['titles']: s['titles'][-1]['end_stored_page']=r['stored_page_number']
report={'schema_version':1,'operation':'science_book_part2_identity_and_structure_discovery','subject_id':SID,'source_name':'كتاب العلوم - الجزء الثاني','classification':'educational_source','raw_image_count':len(manifest['images']),'legacy_question_count':sum(len((p.get('ai_questions') or [])) for p in pages),'master_reference_path':'تاسع علوم/علوم_تاسع_الجزء_الثاني','master_manifest_entries':len(master),'matched_source_page_min':min(r['source_page'] for r in rows if r['source_page'] is not None),'matched_source_page_max':max(r['source_page'] for r in rows if r['source_page'] is not None),'exact_sha_identity_count':sum(1 for r in rows if r['exact_sha_identity']),'identity_failures':failures,'identity_status':'VERIFIED' if not failures and all(r['exact_sha_identity'] for r in rows) else 'NOT VERIFIED','sections':sections,'page_map':rows,'lesson_boundaries_status':'NOT VERIFIED','note':'Section/title runs are evidence-backed discovery candidates from exact master identity. They are not automatically promoted to finalized Lesson boundaries.','raw_mutations':0}
OUT.parent.mkdir(parents=True,exist_ok=True); OUT.write_text(json.dumps(report,ensure_ascii=False,indent=2)+'\n',encoding='utf-8')
print(json.dumps({'identity_status':report['identity_status'],'raw_images':report['raw_image_count'],'exact_sha_identity_count':report['exact_sha_identity_count'],'source_page_range':[report['matched_source_page_min'],report['matched_source_page_max']],'sections':len(sections),'master_entries':len(master),'legacy_questions':report['legacy_question_count'],'output':str(OUT.relative_to(ROOT))},ensure_ascii=False))
