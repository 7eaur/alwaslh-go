...
START = "<!-- SCIENCE_EXAM_1446_CHECKPOINT_START -->"
END = "<!-- SCIENCE_EXAM_1446_CHECKPOINT_END -->"
...
def main():
    ...
    if recon.get("individual_exam_model_count") != 13 or recon.get("finalized_exam_page_count") != 39:
        raise SystemExit("Science 1446 model/page count not finalized")
    qmap = recon.get("question_mapping") or {}
    linked = int(qmap.get("exam_linked_structural", -1))
    if linked != 30 or int(qmap.get("review_required", -1)) != 0 or int(qmap.get("unassigned_within_source", -1)) != 0:
        raise SystemExit("Science 1446 question mapping not complete")

    master = json.loads(MASTER_PATH.read_text(encoding="utf-8"))
    sources = master.get("sources") or []
    current_index = next((i for i, s in enumerate(sources) if s.get("id") == SOURCE_ID), None)
    if current_index is None:
        raise SystemExit("Science 1446 source missing from master")
    ...
    source.update({
        ...
        "classification_evidence": "legacy label + 39-page technical verification + full 39-page visual review + explicit source metadata titles for thirteen 3-page model/correction blocks",
        ...
        "technical_verification": {
            ...
            "images_verified": 39,
            "readable": 39,
            "sha256_match_manifest": 39,
            "mime_match_manifest": 39,
    ...
    progress["correction_sheet_candidates"] = int(progress.get("correction_sheet_candidates") or 0) + (13 - old_corrections)
    progress["source_images_technically_verified"] = int(progress.get("source_images_technically_verified") or 0) + (39 - old_images)
    ...
    p = progress
    body = f"""## Reconstruction checkpoint — Grade 9 Science Ministry Exams 1446

- Phase: `CONTENT RECONSTRUCTION + EXAM BOUNDARY DISCOVERY`
- Source `{SOURCE_ID}` — `العلوم نماذج وزارية 1446` completed from source-local evidence.
