...
def main():
    ...
    if manifest.get("counts", {}).get("pages") != 30 or manifest.get("counts", {}).get("questions") != 0:
    ...
    if actual_titles != expected_titles:
        raise SystemExit("page titles do not match the visually verified 10-model source-local sequence")
    ...
    if any((p.get("ai_questions") or []) for p in by_page.values()):
        raise SystemExit("unexpected legacy questions in English 1445 source")
    ...
    for model in range(1, 11):
        ...
        model_id = f"english-1445-exam-{model:02d}"
        models.append({
            "id": model_id,
            ...
            "answer_key": {
                "status": "NOT VERIFIED",
                "reason": "The third page is visibly an electronic correction/result sheet associated with the same model, but available evidence does not establish a standalone official Answer Key artifact."
            },
            "boundary_evidence": [
                ...
                "The source-local three-page pattern is verified independently for English 1445 and is not inherited from another subject."
            ],
